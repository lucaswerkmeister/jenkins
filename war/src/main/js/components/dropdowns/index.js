...
const generateDropdownDetails = (isSubmenu) => {
  return {
    ...
    onShow(instance) {
      ...
      if (
        instance.reference.items ||
        (instance.reference.target && instance.reference.target.items)
      ) {
        generateMenuItems(
          instance.reference.items || instance.reference.target.items()
        );
      } else {
        const href = instance.reference.target
          ? instance.reference.target.href
          : instance.reference.getAttribute("href");
          ...
          ...items.map(function (x) {
            if (x.type === "HEADER") {
              return createElementFromHTML(
                `<p class="jenkins-dropdown__header">${
            ...
            const menuItem =
              createElementFromHTML(`<${tagName} class="jenkins-dropdown__item" href="${
                x.url
              }">
                                      ${
                x.icon
            ...
            if (x.post || x.requiresConfirmation) {
              menuItem.addEventListener("click", () => {
                if (x.requiresConfirmation) {
                  if (confirm((x.text || x.displayName) + ": are you sure?")) {
                    // TODO I18N
